...
namespace ElevatedStopsEnabler.Util
{
    ...
    {
        private const string HarmonyId = "github.com/bloodypenguin/Skylines-ElevatedStopsEnabler";
            ...
            MethodDefinition transpiler = null)
        {
            ...
            {
                throw new Exception(
                    $"Elevated Stops Enabler: prefix, postfix and transpiler are null for method {original.Type.FullName}.{original.MethodName}");
            }

            try
            {
                Debug.Log($"Elevated Stops Enabler: Patching method {original.Type.FullName}.{original.MethodName}");
            ...
            {
                Debug.LogError($"Elevated Stops Enabler: Failed to patch method {original.Type.FullName}.{original.MethodName}");
        ...
        {
            Debug.Log($"Elevated Stops Enabler: Unpatching method {original.Type.FullName}.{original.MethodName}");
        ...
        private static MethodInfo GetOriginal(MethodDefinition original)
        {
            ...
            {
                throw new Exception(
                    $"Elevated Stops Enabler: Failed to find original method {original.Type.FullName}.{original.MethodName}");
            }
        ...
        private static MethodInfo GetPatch(MethodDefinition patch)
        {
            ...
            if (methodInfo == null)
            {
                throw new Exception($"Elevated Stops Enabler: Failed to find patch method {patch.Type.FullName}.{patch.MethodName}");
            }
